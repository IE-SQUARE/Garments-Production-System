...
@app.route('/style_report')
@login_required
def style_report():
    buyer_query = request.args.get('buyer', '').strip()
    style_query = request.args.get('style', '').strip()
    ...
    # Search box-e kichu likhle database theke data niye ashbe
    if buyer_query or style_query:
        cur.execute("""
            SELECT 
                section,
                process_name, 
            ...
            WHERE 
                (%s = '' OR buyer_name ILIKE %s) AND
                (%s = '' OR style_name ILIKE %s)
            GROUP BY section, process_name
            ORDER BY section, process_name
        """, (buyer_query, f'%{buyer_query}%', style_query, f'%{style_query}%'))
        report_data = cur.fetchall()
    
    cur.close()
    conn.close()
    return render_template('style_report.html', report_data=report_data, buyer_query=buyer_query, style_query=style_query)
